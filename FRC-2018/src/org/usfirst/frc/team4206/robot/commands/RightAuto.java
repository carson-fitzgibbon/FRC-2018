    ...
    protected void execute() {
    	if (plateConfig.charAt(1) == 'R') {
    		...
    		if (System.currentTimeMillis() < startTime + 2000) {
        		...
        		Robot.drivetrain.positionDrive((((2*RobotMap.switchToWallFt*Math.PI)/4)/(2*Math.PI*RobotMap.wheelRadiusFt))*4096, 
        									   (((2*(RobotMap.switchToWallFt+RobotMap.robotWidthFt)*Math.PI)/4)/(2*Math.PI*RobotMap.wheelRadiusFt))*4096);
    		} else {
